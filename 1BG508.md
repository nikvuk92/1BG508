# Human Evolution Population structure practical [2025 April]
...
- There is an UPPMAX project and its name is uppmax2025-2-281;
...
- If by any chance you decide to work on your own computer, make sure to have all the necessary programs installed beforehand (on your responisibility).
...
```
/proj/uppmax2025-2-281/1BG508_2025/  
```
...
Ex.:
 ```
    interactive -A uppmax2025-2-281 -M snowy -p node -n 32 -t 02:00:00
 ```
### Moving about:
```
    cd – change directory
...
# SESSION 1 - Merging, Filtering, QC steps
...
## Step 5 - Filtering out related individuals (OPTIONAL - we can skip this for now if you are running short on time)
...
## Step 6 - Plotting the heterozygosities per population (OPTIONAL - you can look at the different heterozygosities in the different populations, perhaps skip for now if you are running low on time)
...
## Step 7 - Data Merging & strand flipping 

The next step would be to start merging your data with comparative datasets. Due to time constraints, we are now skipping this step and continuing Step 8. You can, however, work through this exercise optionally.
...
For this you should have the `RefInd` files (bim, bed, and fam) in the STARTING_POINT folder.
...
Look at the refpops1.fam file, do you recognize some of these population names (You can use Google)?  
...
# SESSION 2 (Wednesday): Population structure inference 
...
Copy the prepared parameter file from the `1_STARTING_POINT` directory to your working folder it's called 
`PopStrucIn1.par`
...
```
scp YOUR_USERNAME@rackham.uppmax.uu.se:/crex/proj/uppmax2025-2-281/YOUR_DIRECTORY/PopStrucIn1_PCA1.pdf . 
```
...
The files you are using as the input for the below-mentioned scripts is the ones found in ```/2_POPULATION_STRUCTURE_INFERENCE/1_MERGED_DATASET``` so make sure you specify the correct name in the scripts.
...
Create a script (using nano for example) from the code below to run Admixture for K = 2-6 with 3 iterations at each K value.
