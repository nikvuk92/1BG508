# Populationgenomik 1BG508 [2025 Version]
...
- There is an UPPMAX project and its name is uppmax2025-3-1;
...
```
/proj/uppmax2025-3-1/1BG508_2025/  
```
...
Ex.:
 ```
    interactive -A uppmax2025-3-1 -M snowy -p node -n 32 -t 02:00:00
 ```
### Moving about:
```
    cd – change directory
...
```
sed 's/\t/ /g' RefInd_ext.bim | grep " C G" >ATCGlist
sed 's/\t/ /g' RefInd_ext.bim | grep " G C" >>ATCGlist
sed 's/\t/ /g' RefInd_ext.bim | grep " A T" >>ATCGlist
sed 's/\t/ /g' RefInd_ext.bim | grep " T A" >>ATCGlist
```

Exclude the CG and AT SNPs form both your reference ind and data

```
plink  --bfile RefInd_ext --exclude ATCGlist --make-bed --out RefInd1_ext2 
...
For this part of the exercize copy the files from ```/proj/uppmax2025-3-1/1BG508_2025/2_POPULATION_STRUCTURE_INFERENCE/2_PCA``` to your directory.
...
```
scp YOUR_USERNAME@rackham.uppmax.uu.se:/crex/proj/uppmax2025-3-1/1BG508_2025/YOUR_DIRECTORY/PopStrucIn1_PCA1.pdf . 
```
